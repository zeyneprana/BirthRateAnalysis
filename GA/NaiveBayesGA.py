...
df = pd.read_csv("../dataset.csv")  # Dosya yolunu güncelle
...
def fitness_function(bitstring):
    ...
    nb = GaussianNB()
    scores = cross_val_score(nb, X_selected, y_np, cv=5, scoring='accuracy', n_jobs=-1)
...
results = cross_validate(nb, X_selected, y_np, cv=10, scoring=scoring)

print("\nNaive Bayes Modeli (GA ile Seçilen Özelliklerle - 10 Katlı CV)\n")
