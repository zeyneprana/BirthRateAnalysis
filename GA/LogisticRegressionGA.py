...
df = pd.read_csv("../dataset.csv")  # Yolunu gerektiği gibi değiştir
...
# 2. GA FITNESS FONKSİYONU (Logistic Regression ile)
def fitness_function(bitstring):
    ...
    scores = cross_val_score(model, X_selected, y_np, cv=5, scoring='accuracy')
...
# 6. SEÇİLEN ÖZELLİKLERLE X MATRİSİ
X_selected = X_np[:, selected_indices]

# 7. Logistic Regression VE METRİKLER
...
results = cross_validate(lr, X_selected, y_np, cv=10, scoring=scoring)

print("\nLogistic Regression Modeli (GA ile Seçilen Özelliklerle - 10 Katlı CV)\n")
